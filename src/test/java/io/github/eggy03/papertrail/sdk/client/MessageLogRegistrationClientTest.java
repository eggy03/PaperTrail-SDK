...
class MessageLogRegistrationClientTest {
    ...
    private final MessageLogRegistrationEntity dummyEntity = new MessageLogRegistrationEntity(guildId, channelId);

    @Mock
    MessageLogRegistrationService service;

    MessageLogRegistrationClient client;

    @BeforeEach
    void setClient() {
        client = new MessageLogRegistrationClient(service);
    }

    @ParameterizedTest
    @EmptySource
    void testConstructorEmptyBaseUrl(String baseUrl) {
        assertThrows(IllegalArgumentException.class, () -> new MessageLogRegistrationClient(baseUrl));
    }
    ...
    void registerGuild_success_returnsTrue() throws IOException {
        ...
        Call<MessageLogRegistrationEntity> mockedCall = mock(Call.class);

        Response<MessageLogRegistrationEntity> successResponse = Response.success(new MessageLogRegistrationEntity(guildId, channelId));

        when(service.registerGuild(any(MessageLogRegistrationEntity.class))).thenReturn(mockedCall);
        when(mockedCall.execute()).thenReturn(successResponse);

        assertThat(client.registerGuild(guildId, channelId)).isTrue();

        verify(service).registerGuild(any(MessageLogRegistrationEntity.class));
    ...
    void registerGuild_error_returnsFalse() throws IOException {
        ...
        Response<MessageLogRegistrationEntity> errorResponse = Response.error(400, ResponseBody.create("", null));
    ...
    void getRegisteredGuild_success_returnsOptional() throws IOException {
        ...
        Call<MessageLogRegistrationEntity> mockedCall = mock(Call.class);
        @SuppressWarnings("unchecked")
        Response<MessageLogRegistrationEntity> mockedResponse = mock(Response.class);
        ...
        Optional<MessageLogRegistrationEntity> result = client.getRegisteredGuild(anyString());
